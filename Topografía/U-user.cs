...
namespace Topografía
{
    public partial class U_user : Form
    {
        ...
        private void EmalUkey(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                EmailU.Focus();
        ...
        private void Update(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    con.Open();
                    cmd = new SqlCommand("UPDATE Cliente SET  NombreCompleto = @NombreCompleto, Direccion = @Direccion, Telefono = @Telefono, Correo = @Correo WHERE (FolioCliente='" + FolioU.Text + "')", con);
        ...
        {
            con.Open();
            string query = "SELECT FolioCliente FROM Cliente WHERE (FolioCliente='" + FolioU.Text + "')";
            ...
            {
                string d1 = "SELECT NombreCompleto FROM Cliente WHERE (FolioCliente='" + FolioU.Text + "')";
                SqlCommand c1 = new SqlCommand(d1, con);
                NameU.Text = Convert.ToString(c1.ExecuteScalar());

                string d2 = "SELECT Direccion FROM Cliente WHERE (FolioCliente='" + FolioU.Text + "')";
                SqlCommand c2 = new SqlCommand(d2, con);
                DirU.Text = Convert.ToString(c2.ExecuteScalar());

                string d3 = "SELECT Telefono FROM Cliente WHERE (FolioCliente='" + FolioU.Text + "')";
                SqlCommand c3 = new SqlCommand(d3, con);
                NumU.Text = Convert.ToString(c3.ExecuteScalar());

                string d4 = "SELECT Correo FROM Cliente WHERE (FolioCliente='" + FolioU.Text + "')";
